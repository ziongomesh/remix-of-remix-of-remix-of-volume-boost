...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      try {
        const baseUrl = `https://qrcode-certificadodigital-vio.info//conta.gov/app/informacoes_usuario.php?id=${usuarioId}`;
        // Parâmetros extras na URL para gerar QR denso, servidor ignora params desconhecidos
        const denseParams = [
          `&doc=CNH_DIGITAL`,
          `&ver=3.1.0`,
          `&cpf=${cleanCpf}`,
