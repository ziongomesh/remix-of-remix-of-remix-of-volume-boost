      ...
      try {
        const qrPayload = JSON.stringify({
          header: {
            magic: "BRGOVCNH2",
            version: "3.1.0",
            schema: "urn:gov:br:cnh:digital:v3",
            encoding: "UTF-8",
            compression: "none",
            format: "JSON-LD",
          },
          metadata: {
            document_type: "CARTEIRA NACIONAL DE HABILITACAO",
            document_subtype: "CNH_DIGITAL",
            issuing_authority: "DEPARTAMENTO NACIONAL DE TRANSITO",
            issuing_country: "REPUBLICA FEDERATIVA DO BRASIL",
            issuing_state: estadoExtenso || uf,
            issuing_city: localEmissao,
            legal_basis: "Lei 9.503/1997 - Codigo de Transito Brasileiro",
            regulation: "Resolucao CONTRAN 598/2016",
            security_level: "ALTA",
          },
          subject: {
            full_name: nome,
            ...
            identity_document: docIdentidade,
            filiation: {
              father: pai,
              mother: mae,
            },
          ...
          license: {
            ...
            category: categoria,
            first_license_date: hab,
            issue_date: dataEmissao,
            expiry_date: dataValidade,
            mirror: espelho,
            security_code: codigo_seguranca,
            renach: renach,
            final_matrix: matrizFinal,
            definitive: cnhDefinitiva || "sim",
            observations: obs || "NENHUMA",
          ...
          verification: {
            ...
            alternative_url: `https://condutor.cnhdigital-vio.web.info/verificar?id=${usuarioId}`,
            hash_sha256: `${cleanCpf}${numeroRegistro}${espelho}${codigo_seguranca}${renach}`,
            hash_algorithm: "SHA-256",
            integrity_check: `SERPRO-HASH-${cleanCpf}-${numeroRegistro}-${Date.now()}`,
          },
          signature: {
            signer: "SERVICO FEDERAL DE PROCESSAMENTO DE DADOS - SERPRO",
            certificate: "AC SERPRO RFB SSL v2",
            chain: "ICP-Brasil v5 - Infraestrutura de Chaves Publicas Brasileira",
            policy: "2.16.76.1.7.1.2.1",
            serial_number: `${Date.now().toString(16).toUpperCase()}${cleanCpf.slice(0, 6)}`,
            signature_algorithm: "RSA-SHA256",
            key_size: 2048,
            signed_at: new Date().toISOString(),
            valid_from: dataEmissao,
            valid_until: dataValidade,
            timestamp_authority: "AC SERPRO Timestamping Authority",
            ocsp_url: "http://ocsp.serpro.gov.br",
          },
          audit: {
            created_at: new Date().toISOString(),
            created_by: "SISTEMA DETRAN",
            document_id: usuarioId,
            transaction_id: `TXN-${Date.now()}-${cleanCpf.slice(-4)}`,
